...
describe Spree::RoleConfiguration do
  ...
  describe "#register_role" do
    let(:name) { "thing" }
    subject { instance.register_role name, [DummyPermissionSet]}
    ...
    context "when a role for the name exists" do
      before do
        instance.roles = [described_class::Role.new(name, Set.new(existing_roles))]
      end
      ...
      context "when adding duplicate permission sets" do
        ...
        it "does not add duplicate permission sets" do
          subject
          role = instance.roles.detect { |r| r.name == name }
    ...
    context "when a role for the name does not yet exist" do
      ...
      it "sets the roles name accordingly" do
        subject
        expect(instance.roles.first.name).to eql(name)
      end

      it "sets the permission sets accordingly" do
        subject
        expect(instance.roles.first.permission_sets).to match_array([DummyPermissionSet])
      end
    end
  end
  ...
  describe "#activate_permissions!" do
    ...
    context "when the configuration has roles" do
      before do
        instance.roles = [described_class::Role.new(role_name, Set.new([DummyPermissionSet]))]
      end
